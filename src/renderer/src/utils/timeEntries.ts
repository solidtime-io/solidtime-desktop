...
export function getAllTimeEntries(currentOrganizationId: string | null, membershipId: string | null) {
...
export function useTimeEntryUpdateMutation() {
  ...
  return useMutation({
    ...
    onMutate: async (variables: TimeEntry) => {
      await queryClient.cancelQueries({ queryKey: ["timeEntries", currentOrganizationId] });
      queryClient.setQueryData(["timeEntries", currentOrganizationId], (old: TimeEntryResponse) => {
